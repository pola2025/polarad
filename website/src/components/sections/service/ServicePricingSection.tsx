...
export default function ServicePricingSection() {
  return (
    <section className="py-12 lg:py-28 bg-gray-950">
      <div className="container">
        ...
        >
          <h2 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white mb-4 break-keep">
            비즈니스에 맞는 <span className="text-primary-400">플랜</span>을
            선택하세요
          </h2>
        ...
        {/* Pricing Grid – mobile: horizontal scroll, md+: 2-col, lg+: 4-col */}
        <div className="flex gap-4 overflow-x-auto snap-x snap-mandatory scrollbar-hide -mx-4 px-4 pb-2 md:grid md:grid-cols-2 lg:grid-cols-4 md:gap-6 md:overflow-visible md:mx-auto md:px-0 md:pb-0">
          {tiers.map((tier, index) => {
            ...
            return (
              <motion.div
                ...
                className={`relative rounded-2xl border ${colors.border} ${colors.bg} p-5 md:p-6 flex flex-col min-w-[260px] snap-start md:min-w-0 ${
                ...
                <ul className="space-y-3 mb-6 flex-1">
                  {tier.features.map((feature, i) => (
                    <li key={i} className="flex items-start gap-2">
                      {feature.included ? (
                        <Check
                          className={`w-4 h-4 mt-0.5 shrink-0 ${feature.highlight ? "text-amber-400" : "text-emerald-400"}`}
                        />
                      ) : (
                        <X className="w-4 h-4 mt-0.5 shrink-0 text-gray-600" />
                      )}
                      <span
                        className={`text-sm flex items-center gap-1.5 flex-wrap ${feature.included ? (feature.highlight ? "text-amber-300 font-medium" : "text-gray-300") : "text-gray-600"}`}
                      >
                        {feature.text}
                        {feature.badge === "new" && (
