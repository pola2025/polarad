...
export async function GET() {
  const envStatus = {
    ...
    // Instagram (polamkt)
    POLAMKT_INSTAGRAM_ACCESS_TOKEN: process.env.POLAMKT_INSTAGRAM_ACCESS_TOKEN
      ? "설정됨"
      : "없음",
    POLAMKT_INSTAGRAM_ACCOUNT_ID: process.env.POLAMKT_INSTAGRAM_ACCOUNT_ID
      ? "설정됨"
      : "없음",

    // Threads
    THREADS_ACCESS_TOKEN: process.env.THREADS_ACCESS_TOKEN ? "설정됨" : "없음",
    THREADS_USER_ID: process.env.THREADS_USER_ID ? "설정됨" : "없음",

    // Airtable
    AIRTABLE_API_KEY: process.env.AIRTABLE_API_KEY ? "설정됨" : "없음",
    AIRTABLE_BASE_ID: process.env.AIRTABLE_BASE_ID ? "설정됨" : "없음",
  ...
  return NextResponse.json({
    ...
    envStatus,
    recommendation:
      !process.env.HCTI_API_USER_ID || !process.env.HCTI_API_KEY
